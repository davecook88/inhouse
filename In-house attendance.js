var lastMonthStudents = getJsonFromFile("Combined Students","2019-03-01"); //Select title of last josn file in test folder > combined students
var ONLINE_UNITS_DUE_THIS_MONTH = 4; // number of online units to be done in month
...
function insertData(studentObj, ss, coursesObj) {
  ...
  function calculateOnlinePercentage(s) {
    ...
    if (s.folio in lastMonthStudents) {
      var lastMonthStudent = lastMonthStudents[s.folio];
      var lastMonthProgress = 0;
      var thisMonthProgress = 0;
      if (typeof lastMonthStudent.progress[sLevel] != "undefined") lastMonthProgress = lastMonthStudent.progress[sLevel];
      if (typeof student.progress[sLevel] != "undefined") thisMonthProgress = student.progress[sLevel];
      participation = thisMonthProgress - lastMonthProgress;
      
      if (lastMonthStudent.level != sLevel) {
        var previousLevel = lastMonthStudent.level;
        var prevMonthProgress = 0;
        lastMonthProgress = 0;
        if (typeof lastMonthStudent.progress[previousLevel] != "undefined") prevMonthProgress = lastMonthStudent.progress[previousLevel];
        if (typeof student.progress[previousLevel] != "undefined") lastMonthProgress = student.progress[previousLevel];
        participation += lastMonthProgress - prevMonthProgress;
      }
      
    } else {
      participation = student.progress[sLevel];
    }
    
    if(!participation) {
      return 0;
    }
    var finalPercentage = participation / ONLINE_UNITS_DUE_THIS_MONTH;
